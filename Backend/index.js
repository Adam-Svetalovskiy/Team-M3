...
app.get("/getdata/:product_id",(req,res)=>{
    const product_id=req.params.product_id;
    let sqll=`select * from products where product_id=${product_id}`;
...
app.get("/getaddress/:user_id",(req,res)=>{
    const user_id=req.params.user_id
    let sqll=`select * from user_data where user_id=${user_id}`;
    db.query(sqll,(err,result)=>{
        if(err)
        {
            console.log(err)
...
app.get("/myorder/:order_id",(req,res)=>{
   ...
   //let sqll=`SELECT * FROM orderitems,products WHERE orderitems.productid = products.id && orderitems.order_id=${user_id}`
    db.query(sqll,(err,result)=>{
        if(err)
        {
            console.log(err)
...
app.post("/addaddress",(req,res)=>{
    const data={
       ...
       primary_payment:req.body. primary_payment,
   ...
   const REDIRECT_URL = "http://localhost:3000/success";
            ...
            // console.log(redirectUrl)
                    ...
                    const data={
                        ...
                        payment_id:responseData.payment_request.id
                ...
                if(result.length > 0)
                {
                 bcrypt.compare(password,result[0].password,(errr,response)=>{
                     if(response)
                     {
                        ...
                        const token=jwt.sign({id},"ecommerce",{
                            expiresIn:60*60*24,
                        })
